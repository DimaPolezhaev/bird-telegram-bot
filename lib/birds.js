...
export async function getRandomBirdData() {
  try {
    ...
    const facts = await generateBirdFactsWithDeepSeek(birdName, wikiData.extract);
...
async function generateBirdFactsWithDeepSeek(birdName, description) {
  try {
    ...
    // ✅ ДОБАВЬ ПРОВЕРКУ ОТВЕТА
    if (!data.choices || !data.choices[0] || !data.choices[0].message) {
      console.log('❌ Неверный формат ответа от DeepSeek:', data);
      return getDefaultFacts(birdName);
    }
    
    const factsText = data.choices[0].message.content;
    console.log(`✅ DeepSeek ответ получен: ${factsText.substring(0, 100)}...`);
    ...
    if (facts.length === 0) {
      console.log('❌ Не удалось извлечь факты из ответа');
      return getDefaultFacts(birdName);
    }
