...
async function generateBirdFactsWithGemini(birdName, description) {
  try {
    ...
    // ✅ УЛУЧШЕННАЯ ПРОВЕРКА ОТВЕТА GEMINI
    if (!data.candidates || 
        !data.candidates[0] || 
    ...
    const factsText = data.candidates[0].content.parts[0].text;
    console.log(`✅ Gemini ответ получен: ${factsText.substring(0, 100)}...`);
    ...
    // Если не получилось извлечь факты, используем дефолтные
    if (facts.length === 0) {
      console.log('❌ Не удалось извлечь факты из ответа Gemini');
      return getDefaultFacts(birdName);
    }
...
async function getFallbackBirdData() {
  ...
  const facts = await generateBirdFactsWithGemini(bird.name, bird.description); // ✅ ИСПРАВЛЕНО
