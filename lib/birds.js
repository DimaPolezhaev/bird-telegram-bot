...
${description ? `Информация о птице: ${description}` : ''}

ТРЕБОВАНИЯ:
- Каждый факт должен начинаться с "•"
- Факты должны быть 30-80 символов
...
ВАЖНО: Только факты, без пояснений!`;
    ...
    if (!data.candidates || !data.candidates[0] || !data.candidates[0].content) {
      console.log('❌ Неверный ответ от Gemini');
    ...
    const facts = text.split('\n')
      ...
      .filter(fact => fact.length > 15 && fact.length < 100)
      .slice(0, 3);
