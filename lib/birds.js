...
// Разные модели Gemini для перебора
const GEMINI_MODELS = [
...
1. Ссылка должна быть НАПРЯМУЮ на изображение (заканчивается на .jpg, .jpeg, .png)
...
`;
    ...
    for (const model of GEMINI_MODELS) {
      try {
        ...
        if (!res.ok) {
          console.log(`   ⚠️ Gemini model ${model} failed: ${res.status}`);
        ...
        if (imageUrl.startsWith('http') && 
          ...
          console.log(`   ✅ Gemini ${model} предложил фото: ${imageUrl.substring(0, 80)}...`);
          return imageUrl;
        }
        ...
        if (modelError.name === 'AbortError') {
          ...
          console.log(`   ⚠️ Ошибка модели ${model} для поиска фото: ${modelError.message}`);
        }
        continue; // Пробуем следующую модель
      }
    }
    ...
    console.log(`   ❌ Все модели Gemini не сработали для поиска фото: ${error.message}`);
    return null;
  }
}
...
`;
    ...
    console.log(`📝 Все модели не сработали, использую факт как описание для: ${birdName}`);
...
`;
    ...
    for (const model of GEMINI_MODELS) {
      try {
        ...
        if (!birdName || birdName.length < 2) {
          console.log(`⚠️ Модель ${model} вернула некорректное название: "${birdName}"`);
          continue;
        }
        
        // Проверка на общее название
        if (isGeneralFamilyName(birdName)) {
          console.log(`⚠️ Модель ${model} вернула слишком общее название: "${birdName}"`);
          continue;
        }
        
        // Проверка на экзотическую птицу
        if (isExoticBird(birdName)) {
          console.log(`⚠️ Модель ${model} вернула экзотическую птицу: "${birdName}"`);
          continue;
        }
        
        const normalizedBird = normalizeBirdName(birdName);
        if (existingBirdsSet.has(normalizedBird)) {
          console.log(`⚠️ Модель ${model} вернула птицу, которая уже есть в базе: "${birdName}"`);
          continue;
        }
        
        // Проверяем наличие фото
        console.log(`🔍 Модель ${model} предложила: "${birdName}", проверяю фото...`);
        const hasPhoto = await hasRealPhoto(birdName);
        
        if (!hasPhoto) {
          console.log(`⚠️ У птицы от модели ${model} нет фото: "${birdName}"`);
          ...
          for (const similarBird of similarBirds) {
            ...
            if (!existingBirdsSet.has(normalizedSimilar)) {
              ...
              if (hasSimilarPhoto) {
                console.log(`✅ Модель ${model}: нашел похожую птицу с фото: ${similarBird}`);
                return similarBird;
              }
            }
          }
        ...
        console.log(`✅ Модель ${model} сгенерировала подходящую птицу с фото: "${birdName}"`);
        return birdName;
    ...
    console.log('❌ Все модели не сгенерировали подходящую птицу');
    return null;
...
`;
    ...
    console.log('❌ Все модели Gemini не сработали, использую умное перефразирование');
...
export async function generateQuiz() {
  try {
    ...
    if (birdsWithFacts.length < 4) {
      console.log(`❌ Недостаточно птиц с фактами (нужно 4, есть ${birdsWithFacts.length})`);
      return await generateFallbackQuiz();
    }
    ...
    if (!question) {
      console.log('❌ Не удалось сгенерировать вопрос');
      return await generateFallbackQuiz();
    }
