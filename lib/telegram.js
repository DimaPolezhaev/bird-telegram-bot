...
export async function sendSundayQuiz() {
  try {
    ...
    console.log(`📨 Отправляю опрос в канал: ${quizData.question.substring(0, 50)}...`);
    ...
    const requestBody = {
      ...
      explanation: `Правильный ответ: <b>${quizData.correctBird}</b>\n\n` +
                   `🐦 Узнавайте больше о птицах каждый день в нашем канале!`,
      explanation_parse_mode: "HTML"
      // УБРАНО: open_period: 86400 - таймер авто-закрытия
    };
    ...
    if (!result.ok) {
      ...
      // Если ошибка связана с таймером, пробуем без explanation
      if (result.description && result.description.includes('open_period')) {
        console.log('🔄 Пробую отправить без explanation...');
        ...
        const retryBody = {
          ...
          type: "quiz",
          correct_option_id: quizData.correctIndex
          // Убрали explanation тоже, чтобы не было конфликтов
        };
